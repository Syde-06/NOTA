...
export default function PreviewScreen({ route, navigation }) {
  ...
  const summary = useMemo(
    () => buildHighlightSummary(doc?.highlights || {}),
    [doc]
  );
    ...
    <SafeAreaView style={[styles.safe, { backgroundColor: C.bg }]}>
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity
          ...
          onPress={() => navigation?.navigate('Export', { doc })}>
      ...
      <ScrollView
        ...
        {ROLE_DEFINITIONS.map((role) => (
          <View
            ...
            <Text style={[styles.legendCount, { color: C.sub }]}>
              {summary[role.id]}
            </Text>
          </View>
        ))}
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.content}>
        <Text style={[styles.docName, { color: C.text }]}>
          {doc?.title || 'Document Preview'}
        </Text>
        <Text style={[styles.docSubMeta, { color: C.sub }]}>
          {doc?.pages || 1} pages ·{' '}
          {doc?.highlightCount || Object.keys(doc?.highlights || {}).length}{' '}
          highlights · Auto-structured
        </Text>
        ...
        {sections.length > 0 ? (
          sections.map((section) => (
            <View
              ...
              style={[
                styles.block,
                {
                  borderLeftColor: section.roleDef.color,
                  backgroundColor: section.roleDef.soft,
                },
              ]}>
              <View style={styles.blockHeader}>
                <View
                  style={[
                    styles.blockBadge,
                    { backgroundColor: section.roleDef.color },
                  ]}>
                  <Text style={styles.blockBadgeText}>
                    {section.roleDef.emoji}
                  </Text>
                </View>
                <Text
                  style={[styles.blockLabel, { color: section.roleDef.color }]}>
                  {section.roleDef.label}
                </Text>
              </View>
              <Text
                style={[
          ...
          <View style={[styles.emptyCard, { backgroundColor: C.card }]}>
            ...
            <Text style={[styles.emptySub, { color: C.sub }]}>
              Add highlights in the workspace to build a richer preview.
            </Text>
          </View>
        )}
        ...
        <View
          ...
          <View style={styles.aiInfo}>
            ...
            <Text style={styles.aiSub}>
              This view groups your tagged content into a clean study sheet.
            </Text>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  scroll: { flex: 1 },
  content: { padding: 16 },
  docName: { fontSize: 20, fontWeight: '800', marginBottom: 4 },
  docSubMeta: { fontSize: 13, marginBottom: 20 },
  block: {
    borderRadius: 14,
    padding: 16,
    marginBottom: 10,
